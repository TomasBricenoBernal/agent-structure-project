...
- Cross-tool delivery: `CLAUDE.md` stays modular (`@import`, Claude Code expands it); `AGENTS.md` is GENERATED by `planning/scripts/build_agents.py`, which flattens all `.agents/protocol/*.md` into plain text so Codex and opencode (which do not expand imports) get the full protocol. Regenerate `AGENTS.md` after editing any module. Do not hand-edit `AGENTS.md`.
- A committed pre-commit hook in `.githooks/` regenerates and re-stages `AGENTS.md` on every commit. Enable per clone with `git config core.hooksPath .githooks`. This repo already has it enabled.
