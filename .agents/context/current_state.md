...
- Task documentation and QA review rules can now be validated locally with `planning/scripts/validate_task_docs.py` using policy from `.agents/config/review_rules.json`.
...
- Working data moved under a single visible `planning/` folder: `planning/tasks/`, `planning/features/`, `planning/docs/`, `planning/scripts/`. Root keeps only the tool-mandated entries (`CLAUDE.md`, `AGENTS.md`, `.agents/`, `.claude/`, `.gitignore`, `README.md`). The validator resolves repo ROOT three levels up and reads `planning/tasks/active_task.md`.
