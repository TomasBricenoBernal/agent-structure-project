...
This file is the global protocol for Claude in this repository.

Use `.claude/agents/*.md` only as role-specific specialization files.

If `CLAUDE.md`, `AGENTS.md`, and `.claude/agents/*.md` ever diverge:

- Follow `CLAUDE.md` and `AGENTS.md` for repository-wide rules.
- Use `.claude/agents/*.md` only for role-specific behavior.
- Keep both top-level files synchronized after protocol changes.
